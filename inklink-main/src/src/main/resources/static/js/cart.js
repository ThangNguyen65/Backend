    ...
    if (response.status == "Success") {
        ...
        for (let i = 0; i < lineItems.length; i++) {
            ...
            let divStr = `<tr id="ctRow${i}">
                    ...
                    <td id="ctPrice${i}">${lineItems[i].price.toFixed(3)}đ</td>
        ...
        tableBody.html(str);
        $("#ctTotal").text(total.toFixed(3)+'đ');
...
function processCheckout(response) {
    if (response.status == "Success") {
        ...
        $("#puTotalPrice").text("" + cart.total.toFixed(3)+'đ');
...
function doPayment() {
    ...
    if (erm.length == 0) {
        ...
        showMessage("Giá trị mẫu, nhưng đây là nơi bạn đợi cho đến khi bạn thanh toán trong tùy chọn M-Pesa và khi chúng tôi nhận được khoản thanh toán, bạn sẽ tiến hành tải xuống trang. Đợi 2 giây, chờ đã, bạn sẽ không đọc được tin nhắn này trước khi nó thay đổi.");
    } else {
        showErrors(erm);
    }
}
...
function showTwoSteps() {
    $("#puStep1Header").text("Bước 1 của 2");
    $("#puStep2Header").text("Bước 2 của 2");
}

function showThreeSteps() {
    $("#puStep1Header").text("Bước 1 của 3");
    $("#puStep2Header").text("Bước 2 của 3");
    $("#puStep3Header").text("Bước 3 của 3");
}
